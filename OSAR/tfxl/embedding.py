    ...
    def compute_output_shape(self, input_shape):
        ...
        if self.return_embeddings:
            if self.div_val == 1:
                ...
                output_shape += [K.int_shape(embed)
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.div_val == 1:
            ...
            for i in range(len(self.cutoffs) - 1):
                ...
                mask = K.cast(low <= inputs, K.floatx()) * \
                    K.cast(inputs < high, K.floatx())
                selected = K.gather(
                    self.embeddings[i], (inputs - low) * K.cast(mask, 'int32'))
