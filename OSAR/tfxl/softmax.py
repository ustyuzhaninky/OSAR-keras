            ...
            for i in range(len(self.cutoffs) - 1):
                ...
                if self.bind_embeddings[i]:
                    ...
                    self.embeddings.append(self.add_weight(
                        shape=(self.cutoffs[i + 1] -
                               self.cutoffs[i], embed_dim),
                        initializer=self.embeddings_initializer,
                        regularizer=self.embeddings_regularizer,
                        constraint=self.embeddings_constraint,
        ...
        if self.div_val == 1:
            ...
            for i in range(len(self.cutoffs) - 1):
                ...
                if cluster_probs is None:
                    ...
                    if self.use_bias:
                        cluster_probs = K.bias_add(
                            cluster_probs, self.bias_cluster)
                    cluster_output = K.concatenate(
                        [cluster_output, cluster_probs], axis=-1)
                    cluster_output = tf.keras.activations.softmax(
                    ...
                    cluster_output = cluster_output * \
                        K.expand_dims(cluster_probs[..., i - 1])
