...
def build_transformer_xl(units,
    ...
    """
    token_input = tf.keras.layers.Input(shape=(target_len,), name='Input-Token')
    memory_length_input = tf.keras.layers.Input(shape=(1,), name='Input-Memory-Length')
    ...
    if 0.0 < dropout < 1.0:
        outputs[-1] = tf.keras.layers.Dropout(rate=dropout, name='Output-Dropped')(outputs[-1])
