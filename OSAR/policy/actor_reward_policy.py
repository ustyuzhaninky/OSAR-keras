...
__all__ = ['ActorRewardPolicy',]


@gin.configurable
class ActorRewardPolicy(tf_policy.TFPolicy):
  ...
  def __init__(self,
    ...
    if observation_and_action_constraint_splitter is not None:
      if len(tf.nest.flatten(action_spec)) > 1 or (
        ...
        raise NotImplementedError(
            'Action constraints for ActorRewardPolicy are currently only supported '
    ...
    super(ActorRewardPolicy, self).__init__(
  ...
  def _apply_actor_network(self, observation, reward, step_type, policy_state,
                           mask=None):
    if self._observation_normalizer:
      observation = self._observation_normalizer.normalize(observation)
    if mask is None:
      return self._actor_network(
          observation, reward=reward, step_type=step_type, network_state=policy_state,
