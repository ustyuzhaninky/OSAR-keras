...
__all__ = ['OsarDqnAgent', 'TrialAgent']
...
@gin.configurable
class OsarDqnAgent(tf_agent.TFAgent):
    """An osar-dqn Agent.
    """
    ...
    def  __init__(
        ...
        super(OsarDqnAgent, self).__init__(
            time_step_spec,
            action_spec,
            policy,
            collect_policy,
    ...
    def _train(self, experience, weights):
        with tf.GradientTape() as tape:
            loss_info = self._loss(
                experience,
                pu_errors_loss_fn=self._td_errors_loss_fn,
                gamma=self._gamma,
    ...
    def _loss(self,
              experience,
              pu_errors_loss_fn=common.element_wise_huber_loss,
              gamma=1.0,
        ...
        with tf.name_scope('loss'):
            ...
            td_loss = valid_mask * pu_errors_loss_fn(q_values, td_targets)
...
class TrialAgent(tf_agent.TFAgent):
    ...
    def __init__(
            ...
            exploration_noise_std: types.Float = 0.1,
            update_period: types.Int = 1,
            pu_errors_loss_fn: Optional[types.LossFn] = None,
            gamma: types.Float = 1.0,
            batch_size: types.Int = 1,
            epsilon_greedy: types.FloatOrReturningFloat = 0.1,
            n_step_update: int = 1,
        ...
        """Creates a Trial Agent.
            ...
            time_step_spec: A `TimeStep` spec of the expected time_steps.
            action_spec: A nest of BoundedTensorSpec representing the actions.
            disctiminator: A tf_agents.network.Network to be used by the agent as a `Discriminator`. The
                network will be called with call(observation, step_type).
            expert: A tf_agents.network.Network to be used by the agent as an `Expert` selector. The
            ...
            exploration_noise_std: Scale factor on exploration policy noise.
            update_period: Period for the optimization step on actor network.
            pu_errors_loss_fn:  A function for computing the TD errors loss. If None,
                a default value of elementwise huber_loss is used.
            gamma: A discount factor for future rewards.
            batch_size: Batch size of the training inputs.
            epsilon_greedy: probability of choosing a random action in the default
        ...
        if observation_and_action_constraint_splitter:
            net_observation_spec, _ = observation_and_action_constraint_splitter(
                net_observation_spec)
        disctiminator.create_variables(net_observation_spec)

        self._disctiminator = disctiminator
        self._expert = expert

        self._check_network_output(self._disctiminator, 'disctiminator')
        self._check_network_output(self._expert, 'expert')
        ...
        self._epsilon_greedy = epsilon_greedy
        self._exploration_noise_std = exploration_noise_std
        self._pu_errors_loss_fn = (
            pu_errors_loss_fn or common.element_wise_huber_loss)
        self._gamma = gamma
        self._reward_scale_factor = reward_scale_factor
        self._gradient_clipping = gradient_clipping
        self._n_step_update = n_step_update
        ...
        if self._disctiminator.state_spec and n_step_update != 1:
            raise NotImplementedError(
                'DqnAgent does not currently support n-step updates with stateful '
                'networks (i.e., RNNs), but n_step_update = {}'.format(n_step_update))

        train_sequence_length = (
            n_step_update+1 if not self._disctiminator.state_spec else None)
    ...
    def _check_network_output(self, net, label):
        """Check outputs of disctiminator and expert against expected shape.
        Subclasses that require different network outputs should override
        this function.
        Args:
        net: A `Network`.
        label: A label to print in case of a mismatch.
    ...
    def _setup_policy(self, time_step_spec, action_spec,
        ...
        expert_policy = TrialPolicy(
            time_step_spec,
            action_spec,
            network=self._expert,
            expert=True,
            observation_and_action_constraint_splitter=(
                self._observation_and_action_constraint_splitter))
        self._expert_greedy_policy = greedy_policy.GreedyPolicy(expert_policy)

        return policy, collect_policy

    def _compute_discriminator_values(self, time_steps, actions, training=False):
        network_observation = time_steps.observation
        network_reward = time_steps.reward
        ...
        values, _ = self._disctiminator(network_observation,
                                    reward=network_reward,
                                    step_type=time_steps.step_type,
                                    training=training)
        # Handle action_spec.shape=(), and shape=(1,) by using the multi_dim_actions
        ...
        return common.index_with_actions(
            values,
            tf.cast(actions, tf.int32),
            multi_dim_actions=multi_dim_actions), values

    def _compute_expert_values(self, next_time_steps, info, training=False):
        del info
        ...
        expert_values, _ = self._expert(
            network_observation, reward=network_reward, step_type=next_time_steps.step_type)
        batch_size = (
            expert_values.shape[0] or tf.shape(expert_values)[0])
        dummy_state = self._policy.get_initial_state(batch_size)
        # Find the greedy actions using our greedy policy. This ensures that action
        # constraints are respected and helps centralize the greedy logic.
        greedy_actions = self._expert_greedy_policy.action(
            next_time_steps, dummy_state).action

        # Handle action_spec.shape=(), and shape=(1,) by using the multi_dim_actions
        # param. Note: assumes len(tf.nest.flatten(action_spec)) == 1.
        multi_dim_actions = len(
            tf.nest.flatten(self._action_spec)[0].shape) > 0
        return common.index_with_actions(
            expert_values,
            greedy_actions,
            multi_dim_actions=multi_dim_actions), expert_values
    ...
    def _train(self, experience, weights):
        with tf.GradientTape() as tape:
            loss_info = self._loss(
                experience,
                pu_errors_loss_fn=self._pu_errors_loss_fn,
                gamma=self._gamma,
                reward_scale_factor=self._reward_scale_factor,
                weights=weights,
                training=True)
        ...
        tf.debugging.check_numerics(loss_info.loss, 'Loss is inf or nan')
        variables_to_train = self._disctiminator.trainable_weights + self._expert.trainable_weights
        non_trainable_weights = self._disctiminator.non_trainable_weights + \
            self._expert.non_trainable_weights
        assert list(variables_to_train), "No variables in the agent's network."
        grads = tape.gradient(loss_info.loss, variables_to_train)
        # Tuple is used for py3, where zip is a generator producing values once.
        grads_and_vars = list(zip(grads, variables_to_train))
    ...
    def _loss(self,
        ...
        squeeze_time_dim = not self._disctiminator.state_spec
        if self._n_step_update == 1:
            time_steps, policy_steps, next_time_steps = (
                trajectory.experience_to_transitions(experience, squeeze_time_dim))
            actions = policy_steps.action
        ...
        with tf.name_scope('loss'):
            ...
            discriminator_values, discriminator_logits = self._compute_discriminator_values(
                time_steps, actions, training=training)

            expert_values, expert_logits = self._compute_expert_values(
                time_steps, policy_steps.info, training=training)
            # This applies to any value of n_step_update and also in the RNN-DQN case.
            # In the RNN-DQN case, inputs and outputs contain a time dimension.
            # pu_targets = compute_td_targets(
            #     expert_values,
            #     rewards=reward_scale_factor * next_time_steps.reward,
            #     discounts=gamma * next_time_steps.discount)
            ...
            # (discriminator_values - pu_targets)
            pu_error = valid_mask * (discriminator_logits - expert_logits)

            pu_loss = valid_mask * pu_errors_loss_fn(discriminator_logits, expert_logits)#(discriminator_values, pu_targets)

            if nest_utils.is_batched_nested_tensors(
                    time_steps, self.time_step_spec, num_outer_dims=2):
                # Do a sum over the time dimension.
                pu_loss = tf.reduce_sum(input_tensor=pu_loss, axis=1)
            ...
            agg_loss = common.aggregate_losses(
                per_example_loss=pu_loss,
                sample_weight=weights,
                regularization_loss=self._disctiminator.losses)
            total_loss = agg_loss.total_loss

            losses_dict = {'pu_loss': agg_loss.weighted,
                           'reg_loss': agg_loss.regularization,
                           'total_loss': total_loss}
            ...
            if self._summarize_grads_and_vars:
                with tf.name_scope('Variables/'):
                    for var in self._disctiminator.trainable_weights:
                        tf.compat.v2.summary.histogram(
                            name=var.name.replace(':', '_'),
                            data=var,
                            step=self.train_step_counter)
            ...
            if self._debug_summaries:
                diff_values = discriminator_values - expert_values
                common.generate_tensor_summaries('pu_error', pu_error,
                                                 self.train_step_counter)
                common.generate_tensor_summaries('pu_loss', pu_loss,
                                                 self.train_step_counter)
                common.generate_tensor_summaries('discriminator_values', discriminator_values,
                                                 self.train_step_counter)
                common.generate_tensor_summaries('expert_values', expert_values,
                                                 self.train_step_counter)
                common.generate_tensor_summaries('diff_values', diff_values,
                                                 self.train_step_counter)

            return tf_agent.LossInfo(total_loss, dqn_agent.DqnLossInfo(td_loss=pu_loss,
                                                                       td_error=pu_error))
