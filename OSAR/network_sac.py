...
@gin.configurable
class OSARAgent(actor_distribution_network.ActorDistributionNetwork):
    """OSAR network."""
    ...
    def call(self,
        ...
        def call_projection_net(proj_net):
            distribution, _ = proj_net(
                q_value, outer_rank, training=training, mask=mask)
            return distribution
...
@gin.configurable
class OSARCritic(critic_network.CriticNetwork):
    """OSAR critic."""
    ...
    def __init__(self,
        ...
        super(OSARCritic, self).__init__(
            input_tensor_spec,
            name=name)
        ...
        critic_layer = tf.keras.layers.Dense(
            1,
            activation=None,
            kernel_initializer=tf.random_uniform_initializer(
                minval=-0.03, maxval=0.03),
        ...
        self._circuit = circuit
        self._repeater = gru
        self._critic_layer = critic_layer
        self._critic_memory = self.add_weight(
    ...
    def call(self,
        ...
        reward_memory = self._critic_memory

        state = tf.expand_dims(state, axis=0)
        reward = tf.cast(tf.expand_dims(tf.expand_dims(
            reward, axis=-1), axis=-1), dtype=tf.float32)
        context = K.concatenate(
            [state,
             reward_memory,
             reward], axis=-1)
        context = self._context_generator(context, training=training)
        distance, importance, context_updated = self._circuit(
            context, training=training, frozen=self.frozen)
        context = K.concatenate(
            [distance, importance, context_updated], axis=-1)

        action = self._repeater(context)

        critic_value = self._critic_layer(action, training=training)

        self.add_update(K.update(self._critic_memory,
                                 K.expand_dims(critic_value, axis=0)))
        
        return critic_value, network_state
